...
vi.mock('../../cloud/webdav', () => ({
  webdavUpload: vi.fn(),
  generateBackupFilename: vi.fn(() => 'pi-config-backup_pi-config_2026-08-17_14-30-00.json'),
}));
...
describe('Backup Command', () => {
  ...
  it('should backup to WebDAV with generated filename', async () => {
    ...
    expect(webdavUpload).toHaveBeenCalledWith(
      testConfig.webdav,
      expect.any(String),
      'pi-config-backup_pi-config_2026-08-17_14-30-00.json'
    );
    expect(mockCtx.ui.notify).toHaveBeenCalledWith(
      expect.stringContaining('Backed up to WebDAV: pi-config-backup_pi-config_2026-08-17_14-30-00.json'),
      'info'
    );
  });
